...
export function injectDashboard(projectRoot: string, config?: any): InjectResult {
  ...
  const isNextApp = existsSync(join(projectRoot, 'app', 'layout.tsx')) ||
    existsSync(join(projectRoot, 'app', 'layout.js'))
  const isNextPages = existsSync(join(projectRoot, 'pages', '_app.tsx')) ||
    existsSync(join(projectRoot, 'pages', '_app.js'))
  const isVite = existsSync(join(projectRoot, 'vite.config.ts')) ||
...
function injectNextAppRouter(root: string, config: any, created: string[], skipped: string[]): void {
  const pageDir = join(root, 'app', 'settings', 'dashboard')
  ...
  writeFileSync(pagePath, NEXTJS_PAGE_TEMPLATE)
  created.push('app/settings/dashboard/page.tsx')
}

// ─── Next.js Pages Router ────────────────────────────────────────────────────

function injectNextPagesRouter(root: string, config: any, created: string[], skipped: string[]): void {
  const pageDir = join(root, 'pages', 'settings')
  const pagePath = join(pageDir, 'dashboard.tsx')
...
function injectVite(root: string, config: any, created: string[], skipped: string[]): void {
  ...
  mkdirSync(pageDir, { recursive: true })
  writeFileSync(pagePath, REACT_PAGE_TEMPLATE)
