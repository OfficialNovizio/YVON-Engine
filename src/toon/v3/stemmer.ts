...
const STEP3_MAP: [RegExp, string][] = [
  [/icate$/, 'ic'], [/ative$/, ''], [/alize$/, 'al'],
  [/iciti$/, 'ic'], [/ical$/, 'ic'], [/ful$/, ''],
  [/ness$/, ''],
];

const STEP4_MAP: [RegExp, string][] = [
  [/al$/, ''], [/ance$/, ''], [/ence$/, ''], [/er$/, ''],
  [/ic$/, ''], [/able$/, ''], [/ible$/, ''], [/ant$/, ''],
  [/ement$/, ''], [/ment$/, ''], [/ent$/, ''], [/ou$/, ''],
  [/ism$/, ''], [/ate$/, ''], [/iti$/, ''], [/ous$/, ''],
...
export function stem(w: string): string {
  ...
  else if (w.endsWith('e') && measure(w.slice(0, -1)) === 1 && !endsWithCVCNoWXY(w.slice(0, -1))) w = w.slice(0, -1);
...
function measure(s: string): number {
  ...
  for (const c of s) {
    if (isVowel(c)) { if (!inVowel) count++; inVowel = true; }
    else inVowel = false;
  }
  return count;
}

function step1bEnd(w: string): string {
  if (w.endsWith('at') || w.endsWith('bl') || w.endsWith('iz')) return w + 'e';
  if (endsWithDoubleConsonant(w) && !w.endsWith('l') && !w.endsWith('s') && !w.endsWith('z')) return w.slice(0, -1);
  if (measure(w) === 1 && endsWithCVCNoWXY(w)) return w + 'e';
  return w;
}
...
function endsWithCVCNoWXY(s: string): boolean {
