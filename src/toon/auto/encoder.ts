 ...
 */
export function encodeDocument(content: string, title: string = ''): ToonEncodeResult {
 ...
 */
export function encodeMemory(content: string, agentId: string): ToonEncodeResult {
  const sections = parseSections(content)
  const records: string[] = []
 ...
 */
export function encodePrompt(prompt: string): ToonEncodeResult {
  const records: string[] = []
...
function parseSections(content: string): ParsedSection[] {
  ...
  for (const line of lines) {
    ...
    if (!trimmed) {
      if (currentRecords.length > 0) currentRecords.push('')
      continue
    }
...
function classifyHeader(header: string): string {
  ...
  if (h.includes('decision') || h.includes('choice')) return 'decision'
  if (h.includes('task') || h.includes('todo') || h.includes('action')) return 'task'
  if (h.includes('config') || h.includes('setup') || h.includes('setting')) return 'config'
  if (h.includes('goal') || h.includes('objective') || h.includes('target')) return 'goal'
  if (h.includes('context') || h.includes('background') || h.includes('overview')) return 'context'
  if (h.includes('memory') || h.includes('notes') || h.includes('log')) return 'memory'
  if (h.includes('plan') || h.includes('strategy') || h.includes('roadmap')) return 'plan'
  if (h.includes('result') || h.includes('output') || h.includes('response')) return 'result'
  if (h.includes('error') || h.includes('issue') || h.includes('bug')) return 'error'
  if (h.includes('summary') || h.includes('recap')) return 'summary'
  return 'text'
}

function sectionTypeToChar(type: string): string {
...
export function abbreviateText(text: string): string {
  ...
  const entries = Object.entries(ABBREV_MAP).sort((a, b) => b[0].length - a[0].length)
 ...
 */
export function generateDictionaryString(): string {
  const lines: string[] = ['[TOON DICTIONARY — use these abbreviations in your response to save tokens]']
