...
import { ToonSchema, ToonField, toon } from '../toon'
...
export interface StringStats {
  cardinality: number
  topValues: string[]       // top 3 most frequent
  nullCount: number
  avgLength: number
  minLength: number
  maxLength: number
...
export interface StatProfile {
  ...
  booleanFields: Record<string, { trueRatio: number }>
  nullRatio: number
  outliers: string[]         // "col:row val (z-score)"
  trends: string[]           // "col: ↑12% from baseline"
}

export interface StratifiedPayload {
  header: string             // Layer 1: compact stat block
  top: string                // Layer 2: DENSE top-N rows
  rest: string               // Layer 3: hash references
  refs: Record<string, string> // hash → full row for expansion
  totalTokens: number
}

// ─── Summarize: One-pass statistical profile ─────────────────────────────

export function summarize(rows: Record<string, any>[]): StatProfile {
  const n = rows.length
  ...
  const numBufs: Record<string, number[]> = {}
  const strAcc: Record<string, { values: string[]; lengths: number[]; counts: Record<string, number> }> = {}
  let nullCount = 0
  let trueCounts: Record<string, number> = {}
  let boolCounts: Record<string, number> = {}
...
export function formatStatHeader(profile: StatProfile): string {
  const parts: string[] = []
  ...
  for (const [key, s] of strEntries.slice(0, 3)) {
    ...
    if (s.nullCount > 0 && s.nullCount / Math.max(1, profile.rowCount) > 0.9) continue
...
export function formatTopN(rows: Record<string, any>[], schema: ToonSchema, topN: number = 5): string {
  ...
  return toon.dense(truncated, schema)
}
...
export function stratify(
  ...
  if (n < 20) {
    const dense = toon.dense(rows, schema)
    return {
      header: '',
      top: dense,
      rest: '',
  ...
  const avgStrLen = Object.values(firstRow)
    ...
    .reduce((sum, v) => sum + String(v).length, 0) / Math.max(1, 
      Object.values(firstRow).filter(v => typeof v === 'string').length)
...
let _deltaCache: Map<string, Set<string>> = new Map()

export function injectDelta(
  sessionId: string,
  payload: StratifiedPayload
): StratifiedPayload & { isDelta: boolean; sameTokenCount: number } {
  const allHashes = new Set<string>()
  if (payload.top) {
    allHashes.add(createHash('sha256').update(payload.top).digest('hex').slice(0, 8))
  }
  if (payload.rest) {
    const restHash = createHash('sha256').update(payload.rest).digest('hex').slice(0, 8)
  ...
  const newPayload: StratifiedPayload & { isDelta: boolean; sameTokenCount: number } = {
    header: payload.header,
    top: payload.top,
    rest: payload.rest,
    refs: payload.refs,
...
const _expandStore: Map<string, string> = new Map()

export function storeForExpand(refs: Record<string, string>) {
  for (const [hash, data] of Object.entries(refs)) {
    _expandStore.set(hash, data)
  }
}

export function expand(refHash: string): string | null {
  return _expandStore.get(refHash) || null
}

// ─── Helpers ─────────────────────────────────────────────────────────────

function emptyProfile(): StatProfile {
  return {
...
export function autoSchema(name: string, sample: Record<string, any>): ToonSchema {
