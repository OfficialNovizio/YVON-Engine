...
exports.persistToonCall = persistToonCall;
exports.persistEngineQuery = persistEngineQuery;
exports.persistCompileRecord = persistCompileRecord;
exports.getToonStats = getToonStats;
exports.getEngineStats = getEngineStats;
exports.getCostSummary = getCostSummary;
exports.getAgentEfficiency = getAgentEfficiency;
exports.getWeeklyEfficiency = getWeeklyEfficiency;
exports.getContentTypeEfficiency = getContentTypeEfficiency;
exports.getProviderCosts = getProviderCosts;
exports.getHealthScore = getHealthScore;
exports.getRecentQueries = getRecentQueries;
exports.getCompileHistory = getCompileHistory;
exports.getAnomalies = getAnomalies;
exports.getHistoricalToonCalls = getHistoricalToonCalls;
exports.getHistoricalCieTicks = getHistoricalCieTicks;
let db = null;
function getDb() {
    if (db)
        return db;
    ...
    `);
        ...
        try {
            db.exec(`ALTER TABLE toon_calls ADD COLUMN provider TEXT NOT NULL DEFAULT 'unknown'`);
        }
        catch { }
        try {
            db.exec(`ALTER TABLE toon_calls ADD COLUMN agent_id TEXT`);
        }
        catch { }
        try {
            db.exec(`ALTER TABLE toon_calls ADD COLUMN venture_id TEXT`);
        }
        catch { }
        try {
            db.exec(`ALTER TABLE toon_calls ADD COLUMN task_type TEXT`);
        }
        catch { }
        try {
            db.exec(`ALTER TABLE cie_ticks ADD COLUMN agent_id TEXT`);
        }
        catch { }
        try {
            db.exec(`ALTER TABLE cie_ticks ADD COLUMN provider TEXT`);
        }
        catch { }
        return db;
    }
    catch (e) {
        console.error('[metrics/store] SQLite unavailable — metrics will be in-memory only:', e.message);
        return null;
    }
}
// ─── Persistence ─────────────────────────────────────────────────────────────
function persistToonCall(call) {
    const d = getDb();
    if (!d)
        return;
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(call.timestamp, call.provider || 'unknown', call.model || 'unknown', call.format, call.inputTokens, call.outputTokens, call.bytesBefore, call.bytesAfter, call.costSaved, call.agentId || null, call.ventureId || null, call.taskType || null);
    }
    catch { }
}
function persistEngineQuery(query) {
    const d = getDb();
    if (!d)
        return;
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(query.timestamp, query.provider || 'unknown', query.model || 'unknown', query.agentId || null, query.ventureId || null, query.taskType || null, query.queryHash, query.originalChars, query.injectedChars, query.savingsPercent, query.chunksMatched, query.chunksInjected, query.injectionLevel, query.latencyMs, query.docCount, query.memoryCount);
    }
    catch { }
}
function persistCompileRecord(record) {
    const d = getDb();
    if (!d)
        return;
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(record.timestamp, record.durationMs, record.filesScanned, record.chunksBuilt, record.termsIndexed, record.bpeTokens, record.corpusSizeBytes, record.binSizeBytes, record.error || null);
    }
    catch { }
}
// ─── Aggregation Queries ─────────────────────────────────────────────────────
function getToonStats(sinceHours = 24) {
    const d = getDb();
    if (!d)
    ...
    try {
        ...
        for (const m of byModel)
            bm[m.key] = { calls: m.calls, costSaved: m.cost_saved || 0 };
...
function getEngineStats(sinceHours = 24) {
    const d = getDb();
    if (!d)
        return { totalQueries: 0, avgSavingsPercent: 0, totalOriginalChars: 0, totalInjectedChars: 0, avgLatencyMs: 0, avgChunksMatched: 0, byAgent: {}, byTaskType: {}, savingsTrend: [] };
    const since = Date.now() - sinceHours * 3600 * 1000;
    try {
        ...
        for (const a of byAgent)
            ba[a.agent] = { queries: a.q, avgSavings: Math.round(a.s * 10) / 10 };
        const bt = {};
        for (const t of byTask)
            bt[t.task] = { queries: t.q, avgSavings: Math.round(t.s * 10) / 10 };
        return { totalQueries: row.total || 0, avgSavingsPercent: Math.round((row.avg_sav || 0) * 10) / 10, totalOriginalChars: row.orig || 0, totalInjectedChars: row.inj || 0, avgLatencyMs: Math.round(row.avg_lat || 0), avgChunksMatched: Math.round(row.avg_chunks || 0), byAgent: ba, byTaskType: bt, savingsTrend: trend.map((t) => ({ day: t.day, avgSavings: Math.round(t.s * 10) / 10 })) };
    }
    catch {
        return { totalQueries: 0, avgSavingsPercent: 0, totalOriginalChars: 0, totalInjectedChars: 0, avgLatencyMs: 0, avgChunksMatched: 0, byAgent: {}, byTaskType: {}, savingsTrend: [] };
    }
}
function getCostSummary(sinceHours = 24) {
    const d = getDb();
    if (!d)
        return { byModel: {}, totalSpent: 0, totalSaved: 0, netCost: 0 };
    const since = Date.now() - sinceHours * 3600 * 1000;
    try {
        ...
        let totalSaved = 0;
        for (const r of rows) {
            const cost = (r.in_tok / 1000000) * 3 + (r.out_tok / 1000000) * 15;
...
function getAgentEfficiency(sinceHours = 24) {
    const d = getDb();
    if (!d)
        return [];
    const since = Date.now() - sinceHours * 3600 * 1000;
    try {
        ...
        return rows.map((r) => {
            const grade = r.s >= 95 ? 'A+' : r.s >= 90 ? 'A' : r.s >= 80 ? 'B' : r.s >= 60 ? 'C' : r.s >= 30 ? 'D' : 'F';
            const costEstimate = Math.round(((r.orig / 1000000) * 3 + (r.inj / 1000000) * 15) * 10000) / 10000;
...
function getWeeklyEfficiency(days = 7) {
    const d = getDb();
    if (!d)
        return [];
    ...
    try {
        ...
        return rows.map((r) => ({
            ...
            totalTokens: r.tok || 0, cost: Math.round(((r.tok || 0) / 1000000) * 3 * 10000) / 10000,
...
function getContentTypeEfficiency() {
...
function getProviderCosts(sinceHours = 24) {
    const d = getDb();
    if (!d)
        return [];
    const since = Date.now() - sinceHours * 3600 * 1000;
    try {
        ...
        return rows.map((r) => ({
            ...
            cost: Math.round(((r.in_tok / 1000000) * 3 + (r.out_tok / 1000000) * 15) * 10000) / 10000,
...
function getHealthScore() {
    ...
    try {
        ...
        if (lastCompile) {
            ...
            if (toonStaleness > 3) {
                score -= 10;
                penalties.push({ reason: `Index ${toonStaleness}d stale`, points: -10 });
            }
            else if (toonStaleness > 1) {
                score -= 5;
                penalties.push({ reason: `Index ${toonStaleness}d stale`, points: -5 });
            }
        }
        else {
            score -= 15;
            penalties.push({ reason: 'No index compile found', points: -15 });
        }
        ...
        if (recentAvgSavings && recentAvgSavings.s < 85) {
            driftCount = 1;
            score -= 8;
            penalties.push({ reason: 'Compression below 85% — possible sync drift', points: -8 });
        }
        else if (recentAvgSavings && recentAvgSavings.s < 90) {
            driftCount = 1;
            score -= 3;
            penalties.push({ reason: 'Compression below 90%', points: -3 });
        }
        // Check cost
        ...
        if (todayCost) {
            burnRate = Math.round(((todayCost.in_tok || 0) / 1000000) * 3 + ((todayCost.out_tok || 0) / 1000000) * 15);
            projectedMonthly = Math.round(burnRate * 30);
            if (projectedMonthly > 1000) {
                score -= 5;
                penalties.push({ reason: `Projected $${projectedMonthly}/month`, points: -5 });
            }
        }
    }
    catch { }
...
function getCompileHistory(limit = 20) {
    const d = getDb();
    if (!d)
        return [];
    try {
...
function getAnomalies(sinceHours = 24) {
    const d = getDb();
    if (!d)
        return [];
    const since = Date.now() - sinceHours * 3600 * 1000;
    ...
    try {
        ...
        const totalAvg = agentRows.reduce((s, r) => s + r.tok, 0) / Math.max(1, agentRows.length);
...
function getHistoricalToonCalls(sinceHours) {
    const d = getDb();
    if (!d)
        return [];
    try {
...
function getHistoricalCieTicks(sinceHours) {
    const d = getDb();
    if (!d)
        return [];
    try {
