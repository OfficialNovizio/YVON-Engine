...
function detectProjectType(root) {
    if ((0, fs_1.existsSync)((0, path_1.join)(root, 'app', 'layout.tsx')) || (0, fs_1.existsSync)((0, path_1.join)(root, 'app', 'layout.js')))
        return 'nextjs-app';
    if ((0, fs_1.existsSync)((0, path_1.join)(root, 'pages', '_app.tsx')) || (0, fs_1.existsSync)((0, path_1.join)(root, 'pages', '_app.js')))
        return 'nextjs-pages';
    if ((0, fs_1.existsSync)((0, path_1.join)(root, 'vite.config.ts')) || (0, fs_1.existsSync)((0, path_1.join)(root, 'vite.config.js')))
        return 'vite';
    if ((0, fs_1.existsSync)((0, path_1.join)(root, 'server.ts')) || (0, fs_1.existsSync)((0, path_1.join)(root, 'app.ts')))
        return 'express';
    return 'unknown';
}
function scanTypeScriptInterfaces(root) {
    const interfaces = [];
    const scanDir = (dir) => {
...
function interfaceToSchema(iface) {
    ...
    return {
        type: toonType,
        source: (0, path_1.relative)(process.cwd(), iface.file),
        fields,
        sampleCount: 1,
        occurrences: 1,
    };
}
function autoAbbreviate(name, allFields, index) {
...
function mapTypeScriptType(tsType) {
...
// ─── API Route Scanner ────────────────────────────────────────────────────────
function scanApiRoutes(root) {
    const schemas = [];
    const apiDir = (0, path_1.join)(root, 'app', 'api');
...
function extractApiShapes(content, filePath, root) {
    const results = [];
    ...
    while ((match = sbRe.exec(content)) !== null) {
        results.push({
            type: match[1],
            source: (0, path_1.relative)(root, filePath),
            fields: [],
            sampleCount: 0,
            occurrences: 1,
        });
...
function buildDictionary(root, schemas) {
    ...
    if ((0, fs_1.existsSync)(configPath)) {
        try {
            ...
            if (config.ventures) {
                Object.entries(config.ventures).forEach(([slug, data], i) => {
...
function scanForFiles(dir, extensions, results) {
    if (!(0, fs_1.existsSync)(dir))
        return;
    for (const entry of (0, fs_1.readdirSync)(dir, { withFileTypes: true })) {
        const full = (0, path_1.join)(dir, entry.name);
...
function mapInjectionPoints(scan) {
    const points = [];
    // 1. Claude/LLM route — always inject prompt compression
    const claudePaths = [
        (0, path_1.join)(scan.projectRoot, 'app', 'api', 'claude', 'route.ts'),
        (0, path_1.join)(scan.projectRoot, 'app', 'api', 'claude', 'route.tsx'),
        (0, path_1.join)(scan.projectRoot, 'pages', 'api', 'claude.ts'),
    ];
    ...
    for (const route of apiRoutes) {
        const routePaths = [
            (0, path_1.join)(scan.projectRoot, 'app', 'api', route, 'route.ts'),
            (0, path_1.join)(scan.projectRoot, 'app', 'api', route, 'route.tsx'),
        ];
    ...
    if (scan.documentPaths.length > 0) {
        points.push({
            type: 'document-store',
            path: (0, path_1.join)(scan.projectRoot, '.toon'),
    ...
    if (scan.memoryPaths.length > 0) {
        points.push({
            type: 'memory-store',
            path: (0, path_1.join)(scan.projectRoot, '.toon', 'memory'),
            action: 'create-schema',
...
// ─── Savings Estimator ────────────────────────────────────────────────────────
function estimateSavings(scan) {
    let base = 20; // Baseline savings from schema compression
    if (scan.schemas.length >= 3)
        base += 10;
    if (scan.documentPaths.length >= 10)
        base += 15;
    if (scan.memoryPaths.length >= 5)
        base += 10;
    if (scan.injectionPoints.length >= 5)
        base += 10;
    // Claude route injection is the biggest win
    const hasClaudeInjection = scan.injectionPoints.some(p => p.type === 'claude-route');
    if (hasClaudeInjection)
        base += 15;
